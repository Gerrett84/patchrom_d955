        ...
        if [ "$CUST" != /system/cust ]; then
            ...
            case "$NTCODE" in 
	           ...
	           if [ "$TAG2" != "$CURRENT" ]; then 
    	               ...
    	               DATA_NORMAL=/data/app
    	               
                       if [ -f $CONF/ntcode_unlist_${NTCODE}.cfg ]; then     	       
	                   for apk1 in $(cat $CONF/ntcode_unlist_${NTCODE}.cfg); do
                               `cat ${APPPATH}/${apk1} > ${DATA_NORMAL}/${apk1}`
                                   chown system:system ${DATA_NORMAL}/${apk1}
                                   chmod 644 ${DATA_NORMAL}/${apk1}
                           done
    	               elif [ -f $CONF/ntcode_unlist_FFF.cfg ]; then	       
    	                   for apk2 in $(cat $CONF/ntcode_unlist_FFF.cfg); do
                               `cat ${APPPATH}/${apk2} > ${DATA_NORMAL}/${apk2}`
                               chown system:system ${DATA_NORMAL}/${apk2}
                               chmod 644 ${DATA_NORMAL}/${apk2}
                           done
    	               fi
